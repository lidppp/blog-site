// fps 检测
import {FPSTracker} from "dok-fps"
// 起始和结束角
const START = 0
const END = 2 * Math.PI
...
class Snow {
    ...
    watchFps() {
        FPSTracker.tick();
        if (!this.fpsWatchKey) {
            this.fpsWatchKey = setInterval(() => {
                // console.log("snow FPS",FPSTracker.frameRate)
                if (FPSTracker.frameRate < 30) {
                    this.fpsLowNum++;
                    if (this.fpsLowNum > 10) {
                        clearInterval(this.fpsWatchKey)
                        this.destroy()
                    }
                } else {
                    this.fpsLowNum = 0
